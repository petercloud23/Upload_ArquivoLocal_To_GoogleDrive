...
def upload_basic():
    ...
    try:
        ...
        file_path = file_path = r"C:\Users\Peterson\PycharmProjects\PythonProject\Teste_UploadToDrive.txt" # <-- ALTERE ESTA LINHA
        ...
        # Adiciona a opção de desconectar
        disconnect = input("Deseja desconectar da conta Google? (s/n): ")
        if disconnect.lower() == 's':
            if os.path.exists('token.json'):
                os.remove('token.json')
                print("Conta Google desconectada com sucesso.")
            else:
                print("Nenhuma conta Google conectada encontrada.")
